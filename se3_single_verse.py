...
def get_single_verse(p_previousContext, p_context, p_dbConnectionPool):
    ...
    if (l_idGroup0 == 'OT' or l_idGroup0 == 'NT') and not p_context['n']:
    ...
    if (l_idGroup0 == 'OT' or l_idGroup0 == 'NT') and not p_context['k']:
    ...
    if l_idGroup0 == 'OT' and not p_context['x']:
    ...
    if l_pcVerse == '1':
        ...
        l_previousBook = l_pcBookId
        l_previousChapter = l_pcChapter
        l_previousVerse = int(l_pcVerse) - 1

    # Next
    if l_pcVerse == str(g_bookChapter[l_pcBookId][int(l_pcChapter)]):
        ...
        l_nextBook = l_pcBookId
        l_nextChapter = l_pcChapter
        l_nextVerse = int(l_pcVerse) + 1
    ...
    # 5 adjacent verses
    l_5start = max(int(l_pcVerse) - 2, 1)
    l_5end = min(l_5start + 4, g_bookChapter[l_pcBookId][int(l_pcChapter)])
    if l_5end - l_5start < 4:
        l_5start = max(l_5end - 4, 1)

    l_5Link = ('<a href="" class="svGoPassage TopLink" newBook="{0}" newChapter="{1}" newVerse1="{2}" ' +
               'newVerse2="{3}">{4}</a>').format(l_pcBookId, l_pcChapter, l_5start, l_5end,
                                                 get_user_string(p_context, 'sv_5Neighborhood'))

    # 9 adjacent verses
    l_9start = max(int(l_pcVerse) - 4, 1)
    l_9end = min(l_9start + 8, g_bookChapter[l_pcBookId][int(l_pcChapter)])
    if l_9end - l_9start < 8:
        l_9start = max(l_9end - 8, 1)

    l_9Link = ('<a href="" class="svGoPassage TopLink" newBook="{0}" newChapter="{1}" newVerse1="{2}" ' +
               'newVerse2="{3}">{4}</a>').format(l_pcBookId, l_pcChapter, l_9start, l_9end,
    ...
    try:
        ...
        l_response += '<tr><td colspan="2" class="svInterlinearBox">{0}</td></tr>\n'.format(l_interlinearHtml)
    ...
    if l_idGroup0 == 'OT' and p_context['x']:
        l_query = """
    ...
    if (l_idGroup0 == 'OT' or l_idGroup0 == 'NT') and (p_context['k'] or p_context['n']):
        l_query = """
    ...
    l_response += '<tr><td colspan="2" class="svTranslationsTitle">{0}</td></tr>\n'.format(
    ...
    l_query = """
        ...
        where
            ...
            and V.N_VERSE = {3}
        order by N.N_ORDER
        ;""".format(l_versionVector, l_pcBookId, l_pcChapter, l_pcVerse)
    ...
    try:
        ...
        for l_verseText, l_versionLabel in l_cursor:
            l_response += ('<tr><td class="svVersionLabel">{0}</td>' +
